...
1. 修改代码后，可以视完善程度自主决定是否提交到本地 git 仓库，注意 commit message 需要规范编写，尽量详细；
   - 补充：尽量不要采用回滚得到方式来恢复错误修改的代码，如果前面提交的代码有错，修复后重新提交即可；
2. 构建、测试等工作，建议参考 `./README.md`，采用与之一致的做法;
   - 补充: 编译输出的可执行文件为 `./pz-netlink`，推荐每次修改后执行 build 以确认代码正确性；
