...
type Proxy struct {
	...
	conns    map[net.Conn]*http.Request
	mu       sync.RWMutex
	bytesIn  int64
	bytesOut int64
	running  atomic.Bool
...
func NewProxy(cfg *types.HTTPProxyConfig, client interface {
	Dial(network, addr string) (net.Conn, error)
}) *Proxy {
	return &Proxy{
		config:    cfg,
		sshClient: client,
		conns:     make(map[net.Conn]*http.Request),
	}
}
...
func (p *Proxy) handleConn(client net.Conn) {
	...
	req, err := http.ReadRequest(newReaderBuf(buf[:n]))
	if err != nil {
		return
	}
...
func (p *Proxy) handleConnect(client net.Conn, req *http.Request) {
	...
	remote, err := p.sshClient.Dial("tcp", host)
	if err != nil {
...
func newReaderBuf(b []byte) *readerBuf {
	return &readerBuf{buf: b}
}

func (r *readerBuf) Read(p []byte) (n int, err error) {
	if r.pos >= len(r.buf) {
		return 0, io.EOF
	}
